...
function FilterCountries(event){
    ...
    let Cnty=event.target.value

    let country=countries.find(c=>c.name==Cnty)
    ...
    <div class="relative aspect-[16/9]  w-auto rounded-md md:aspect-auto md:h-[400px]">
              ...
              <div class="absolute inset-0 rounded-md bg-gradient-to-t from-gray-900 to-transparent"></div>
              <div class="absolute bottom-4 left-4 text-left">
                <h1 class="text-lg font-semibold text-white">${country.name}</h1>
                <p class="mt-2 text-sm text-gray-300">
               ${country.borders}
                </p>
                <button class="mt-2 inline-flex cursor-pointer items-center text-sm font-semibold text-white">
...
function PopulateCountry(event){

    let choice=new Set(countries.map(c=>c.name))
